...
print("━"*72)
print("  WAREHOUSE FORKLIFT SAFETY INTELLIGENCE SYSTEM  |  Anas-Maqsood  v2.1")
print("━"*72)
...
_VIDEO_EXTENSIONS = (".mp4", ".avi", ".mov", ".mkv", ".mpeg", ".mpg", ".wmv", ".webm")
...
def find_video() -> str:
    ...
    if len(sys.argv) > 1:
        arg = sys.argv[1]
        if _is_video_file(arg):
            return arg
        if os.path.isfile(arg):
            print(f"[WARN] CLI arg '{arg}' is not a supported video file, falling back to auto-detect")
        else:
    ...
    for pattern in ["*.mp4","*.avi","*.mov","*.mkv","*.mpeg","*.mpg","*.wmv","*.webm"]:
        for c in glob.glob(pattern):
            if _is_video_file(c):
                return c
    return ""
